...
RSpec.describe FmRest::Spyke::Model::Connection do
  describe ".connection" do
    ...
    it { expect(subject.url_prefix.to_s).to eq("https://example.com/fmi/data/v1/databases/TestDB/") }

    it { expect(subject.builder.handlers).to include(FmRest::V1::TokenSession) }
    it { expect(subject.builder.handlers).to include(Faraday::Request::Json) }
    it { expect(subject.builder.handlers).to include(FmRest::Spyke::SpykeFormatter) }
    it { expect(subject.builder.handlers).to include(FmRest::V1::TypeCoercer) }
    it { expect(subject.builder.handlers).to include(Faraday::Response::Json) }
